...
func autoGenerateDocs(dashboardDir string) {
	...
	// Check if docs already exist (check for HTML first, then markdown)
	docsDir := filepath.Join(workspaceRoot, "docs")
	indexPathHTML := filepath.Join(docsDir, "index.html")
	indexPathMD := filepath.Join(docsDir, "index.md")
	docsExist := false
	if _, err := os.Stat(indexPathHTML); err == nil {
		docsExist = true
	} else if _, err := os.Stat(indexPathMD); err == nil {
		docsExist = true
	}
